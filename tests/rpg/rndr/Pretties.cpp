...
Renderer::Pretties::Pretties(const Renderer& _renderer)
...
void Renderer::Pretties::render_post_lighting() {
    ...
    if (options.Shafts != 0) {
        if (renderer.world.check_SkyLight() == true) {
            ...
            renderer.world.get_SkyLight().texA.bind(gl::TEXTURE0);
...
void Renderer::Pretties::render_final_screen() {
    ...
    if (options.FSAA) renderer.pipeline.use_shader(FS_prty_fsaa_last);
...
void Renderer::Pretties::update_options() {
    ...
    if (options.SSAO != 0) {
        ...
        if (options.SSAO == 1) renderer.preprocs(FS_prty_ssao_ssao, "pretty/ssao/ssao_fs", defAspect);
        else renderer.preprocs(FS_prty_ssao_ssao, "pretty/ssao/ssao_fs", "#define HIGH" + defAspect);
    }

    if (options.Shafts != 0) {
        if (options.Shafts == 1) renderer.preprocs(FS_prty_shafts_shafts, "pretty/shafts/shafts_fs");
    ...
    if (options.Shafts != 0 && options.Bloom == true)
        renderer.preprocs(FS_prty_hdr_tones, "pretty/hdr/tones_fs", "#define SHAFTS\n#define BLOOM");
    else if (options.Shafts) renderer.preprocs(FS_prty_hdr_tones, "pretty/hdr/tones_fs", "#define SHAFTS");
    else if (options.Bloom) renderer.preprocs(FS_prty_hdr_tones, "pretty/hdr/tones_fs", "#define BLOOM");
    else renderer.preprocs(FS_prty_hdr_tones, "pretty/hdr/tones_fs");

    if (options.FSAA != 0) {
        if (options.FSAA == 1) renderer.preprocs(FS_prty_fsaa_last, "pretty/fsaa/fxaa_fs", defFPixSize);
