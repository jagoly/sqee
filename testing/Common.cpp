...
constexpr const float EPSILON = std::numeric_limits<float>::epsilon() * 8.f;
...
{
    const Vec4F vecA = { a.x, a.y, a.z, a.w };
    const Vec4F vecB = { b.value.x, b.value.y, b.value.z, b.value.w };
    const float maxLength = maths::max(maths::length(a), maths::length(b.value));
    const float dist = maths::distance(vecA, vecB);
    return (std::abs(dist - 2.f) < EPSILON) || (dist <= EPSILON * (1.f + maxLength));
}
