...
float get_bias(vec3 _normal, vec3 _lightDir) {
    ...
    return clamp(0.005f * magicTan, 0.0f, 0.05f);
}
...
float sample_shadow_p4(vec3 _sc, float _bias, sampler2DShadow _tex) {
    float angle = rand(_sc.xy);
    float s = sin(angle);
    float c = cos(angle);
    float vis = 0.f;
    for (int ind = 0; ind < 4; ind++) {
        vec2 offs = disk4[ind] * (1.f / 256.f);
...
float sample_shadow_p8(vec3 _sc, float _bias, sampler2DShadow _tex) {
    float angle = rand(_sc.xy);
    float s = sin(angle);
    float c = cos(angle);
    float vis = 0.f;
    for (int ind = 0; ind < 8; ind++) {
        vec2 offs = disk8[ind] * (1.f / 256.f);
        offs = vec2(c * offs.x - s * offs.y, c * offs.y + s * offs.x);
        vis += texture(_tex, vec3(_sc.x+offs.x, _sc.y+offs.y, _sc.z - _bias));
    }
    return vis / 8.f;
...
float sample_shadow_p16(vec3 _sc, float _bias, sampler2DShadow _tex) {
    float angle = rand(_sc.xy);
    float s = sin(angle);
    float c = cos(angle);
    float vis = 0.f;
    for (int ind = 0; ind < 16; ind++) {
        vec2 offs = disk16[ind] * (1.f / 256.f);
        offs = vec2(c * offs.x - s * offs.y, c * offs.y + s * offs.x);
        vis += texture(_tex, vec3(_sc.x+offs.x, _sc.y+offs.y, _sc.z - _bias));
    }
    return vis / 16.f;
