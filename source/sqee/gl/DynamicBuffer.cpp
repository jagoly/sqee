...
void DynamicBuffer::allocate_constant(uint _size, const void* _data) const {
    gl::NamedBufferStorage(buf, _size, _data, gl::NONE);
}

void DynamicBuffer::allocate_editable(uint _size, const void *_data) const {
    gl::NamedBufferStorage(buf, _size, _data, gl::DYNAMIC_STORAGE_BIT);
}

void DynamicBuffer::update(uint _offs, uint _size, const void* _data) const {
