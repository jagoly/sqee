...
void Pretties::update_settings() {
    ...
    if (ssao != 0) {
        TEX_ssao.allocate_storage(renderer.INFO.halfSize, false);
        TEX_ssaoB.allocate_storage(renderer.INFO.halfSize, false);
    } else TEX_ssao.delete_object(), TEX_ssaoB.delete_object();

    if (bloom == true) {
        TEX_bloom.allocate_storage(renderer.INFO.qterSize, false);
        TEX_bloomB.allocate_storage(renderer.INFO.qterSize, false);
    } else TEX_bloom.delete_object(), TEX_bloomB.delete_object();

    if (shafts != 0) {
        TEX_shafts.allocate_storage(renderer.INFO.qterSize, false);
    } else TEX_shafts.delete_object();

    TEX_final.allocate_storage(renderer.INFO.fullSize, false);
}
