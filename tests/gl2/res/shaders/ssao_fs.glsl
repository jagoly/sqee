...
const vec3 pSphere[10] = vec3[](vec3(-0.010735935, 0.01647018, 0.0062425877),vec3(-0.06533369, 0.3647007, -0.13746321),vec3(-0.6539235, -0.016726388, -0.53000957),vec3(0.40958285, 0.0052428036, -0.5591124),vec3(-0.1465366, 0.09899267, 0.15571679),vec3(-0.44122112, -0.5458797, 0.04912532),vec3(0.03755566, -0.10961345, -0.33040273),vec3(0.019100213, 0.29652783, 0.066237666),vec3(0.8765323, 0.011236004, 0.28265962),vec3(0.29264435, -0.40794238, 0.15964167));
   ...
   vec3 fres = normalize((texture(texRandnorm, texcoord * offset).xyz * 2.f - 1.f));
   vec4 currentPixelSample = texture(texNorm, texcoord);
   float currentPixelDepth = currentPixelSample.a;

   // current fragment coords in screen space
   vec3 ep = vec3(texcoord, currentPixelDepth);
 // get the normal of current fragment
   vec3 norm = normalize(currentPixelSample.xyz * 2.f - 1.f);
   ...
   for (int i = 0; i < sampleCount; i++) {
      ...
      vec4 occluderFragment = texture(texNorm, se.xy);

      // get the normal of the occluder fragment
      occNorm = normalize(occluderFragment.xyz * 2.f - 1.f);
      ...
      normDiff = (1.f - dot(occNorm, norm));
      // the falloff equation, starts at falloff and is kind of 1/x^2 falling
      bl += step(falloff, depthDifference) * normDiff * (1.f - smoothstep(falloff, strength, depthDifference));
   }

   // output the result
   float ao = 1.f - totStrength * bl * invSamples;
