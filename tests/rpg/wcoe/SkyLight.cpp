...
void SkyLight::refresh() {
    if (PROP_enabled == false) {
        ...
        texA.allocate_storage(uvec3(adjSize, adjSize, 4u), false);
        texB.allocate_storage(uvec3(adjSize, adjSize, 2u), false);
    }
...
void SkyLight::calc(double _accum) {
    ...
    fmat4 viewMatB = glm::lookAt(world.camera->pos-PROP_direction, world.camera->pos, tangent);
