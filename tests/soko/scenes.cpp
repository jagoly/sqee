...
SceneMain::SceneMain(sq::Stage& sta, sq::TextureHolder& texHolder) {
    ...
    textureHolder->add_texture("wall", "res/wall.png");
    textureHolder->add_texture("hole", "res/hole.png");
    textureHolder->add_texture("rock", "res/rock.png");
    textureHolder->add_texture("goal", "res/goal.png");
}
...
void SceneMain::render(float ft) {
    ...
    if (stage->pFace != -1) {
        if      (stage->pFace == 0) playerSprite.setTextureRect({0,0,64,64});
        else if (stage->pFace == 1) playerSprite.setTextureRect({0,64,64,64});
        else if (stage->pFace == 2) playerSprite.setTextureRect({64,0,64,64});
