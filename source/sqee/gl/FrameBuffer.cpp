...
void FrameBuffer::revert_to_defaults() {
    gl::DeleteFramebuffers(1, &fbo);
    gl::GenFramebuffers(1, &fbo);
    drawBufs.clear();
    bufSet.clear();
...
void FrameBuffer::attach(GLenum _attach, TextureCube& _tex, uint _face) {
    bind(gl::FRAMEBUFFER); _tex.bind(); add_buf(_attach);
...
void FrameBuffer::attach(GLenum _attach, Texture2DArray& _tex, uint _index) {
    bind(gl::FRAMEBUFFER); _tex.bind(); add_buf(_attach);
...
void FrameBuffer::attach(GLenum _attach, TextureCubeArray& _tex, uint _index, uint _face) {
    bind(gl::FRAMEBUFFER); _tex.bind(); add_buf(_attach);
    gl::FramebufferTextureLayer(gl::FRAMEBUFFER, _attach, _tex.tex, 0, _index * 6 + _face);
}

void FrameBuffer::attach(GLenum _attach, TextureMutCubeArray& _tex, uint _index, uint _face) {
    bind(gl::FRAMEBUFFER); _tex.bind(); add_buf(_attach);
    gl::FramebufferTextureLayer(gl::FRAMEBUFFER, _attach, _tex.tex, 0, _index * 6 + _face);
}
