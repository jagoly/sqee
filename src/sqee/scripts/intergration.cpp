...
string tostring_fvec2(fvec2 v) { return tfm::format( "vec2(%f, %f)", v.x,v.y); };
...
string tostring_fvec3(fvec3 v) { return tfm::format( "vec3(%f, %f, %f)", v.x,v.y,v.z); };
...
string tostring_fvec4(fvec4 v) { return tfm::format( "vec4(%f, %f, %f, %f)", v.x,v.y,v.z,v.w); };
...
string tostring_fquat(fquat q) { return tfm::format( "quat(%f, %f, %f, %f)", q.w,q.x,q.y,q.z); };
...
void sq::cs_setup_maths(chai::ChaiScript& _cs) {
    ...
    add_class<fvec2>(*m, "vec2", { constructor<fvec2(const fvec2&)>(),
    ...
    add_class<fvec3>(*m, "vec3", { constructor<fvec3(const fvec3&)>(),
    ...
    add_class<fvec4>(*m, "vec4", { constructor<fvec4(const fvec4&)>(),
    ...
    add_class<fquat>(*m, "quat", { constructor<fquat(const fquat&)>(),
