...
void Decal::load_from_spec(const ObjSpec& _spec) {
    ...
    SPEC_IF_STRING("diff") PROP_dPath = _spec.sMap.at("diff")[0];
    SPEC_IF_STRING("norm") PROP_nPath = _spec.sMap.at("norm")[0];
    SPEC_IF_STRING("spec") PROP_sPath = _spec.sMap.at("spec")[0];
}

void Decal::refresh() {
    if (PROP_dPath.empty() == false) {
        const string name = "decals/" + PROP_dPath + "_d";
    ...
    if (PROP_nPath.empty() == false) {
        const string name = "decals/" + PROP_nPath + "_n";
    ...
    if (PROP_sPath.empty() == false) {
        const string name = "decals/" + PROP_sPath + "_s";
